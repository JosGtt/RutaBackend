...
export const validateCriticalEnvVars = (): void => {
  ...
  // En producción, también requerir DATABASE_URL o variables individuales
  if (process.env.NODE_ENV === 'production') {
    if (!process.env.DATABASE_URL) {
      criticalVars.push('DB_HOST', 'DB_USER', 'DB_PASSWORD', 'DB_NAME');
    }
  }
  ...
  if (jwtSecret === 'default-secret' || jwtSecret.includes('your_super_secret')) {
    logger.error({
